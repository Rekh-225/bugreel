...
The README demo (`docs/demo.gif`, full-quality `docs/demo.mp4`) is regenerated by two npm scripts and requires a full-featured ffmpeg (Windows: `winget install --id Gyan.FFmpeg -e`; or set `FFMPEG` to a binary). Stop any server on port 3000 first, then run `npm run build`, `npm run demo:record`, and `npm run demo:encode`.

`demo:record` starts the production server with the test-only CDP port 9333 and records the dashboard only via Playwright `recordVideo` — a visible recording browser opens; do not touch it. It captures two segments (the long replay wait is polled off-camera), writes absolute paths to `docs/demo-raw/segments.json`, and runs a real session saved under `.bugreel/` like any other. `demo:encode` concatenates the segments and produces the mp4 (libx264) and the GIF (two-pass palette, retried at lower fps/width if over 6 MB). Raw segments and verification stills under `docs/demo-raw/` are gitignored.
