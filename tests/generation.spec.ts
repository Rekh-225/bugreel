...
test('missing checkout and external navigation cannot generate a runnable scenario', () => {
  expect(() => generateTest({ actions: actions.slice(0, 2), startUrl: 'http://127.0.0.1:3000/demo-store' })).toThrow('exactly one checkout');
...
test('the generated test is executable by the actual Playwright CLI', async () => {
  test.setTimeout(70_000);
  const directory = path.resolve('.bugreel', 'generation-verification', crypto.randomUUID());
  await fs.mkdir(directory, { recursive: true });
  await fs.writeFile(path.join(directory, 'reproduction.spec.ts'), generateTest({ actions, startUrl: 'http://127.0.0.1:3000/demo-store' }));
