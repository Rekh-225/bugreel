...
test('landing communicates the workflow and has no horizontal overflow', async ({ page }) => {
  ...
  if (process.platform === 'win32') await expect(page.locator('.hero')).toHaveScreenshot('landing-hero.png', { animations: 'disabled' });
...
test('Demo Store has a clear visible failure and fits mobile', async ({ page }) => {
  ...
  if (process.platform === 'win32') await expect(page.locator('.store-grid')).toHaveScreenshot('demo-failure.png', { animations: 'disabled' });
